...
import { getProviderProfile, updateOnlineStatus } from '../../services/supabaseProviderService';
import { getPendingRequestForProvider } from '../../services/marketplaceService';
  ...
  useEffect(() => {
    ...
    if (profile?.id && isOnline) {
      const checkIncomingRequests = async () => {
        try {
          const result = await getPendingRequestForProvider(profile.id);
          setIncomingJob(result.request || null);
        } catch (e) {
          console.error('Error polling incoming requests:', e);
        }
      };

      checkIncomingRequests();
      interval = setInterval(checkIncomingRequests, 5000);
    } else {
      setIncomingJob(null);
    }
    return () => {
          ...
          {incomingJob && (
            <View style={styles.incomingJobCard}>
              <View style={styles.incomingHeader}>
                ...
                <Text style={styles.incomingTimer}>Upfront Fare</Text>
              </View>
              <Text style={styles.incomingDetails}>
                {incomingJob.serviceName} requested by {incomingJob.customerName}
              </Text>
              <Text style={styles.incomingAddress}>📍 {incomingJob.address || 'Karachi, Pakistan'}</Text>
              <Text style={styles.incomingPrice}>Rs. {incomingJob.totalAmount || 500}</Text>
              <View style={styles.incomingButtons}>
                <TouchableOpacity
                  style={styles.rejectBtn}
                  onPress={async () => {
                    const { rejectJob } = require('../../services/providerJobService');
                    await rejectJob(incomingJob.id);
                    setIncomingJob(null);
                  }}
                >
                ...
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    const { acceptJobRequest } = require('../../services/realtimeJobFlowService');
                    const result = await acceptJobRequest(incomingJob.id, profile.id);
                    if (result.success) {
                      setIncomingJob(null);
                      navigation.navigate('ActiveJob', { jobId: incomingJob.id });
                    } else {
